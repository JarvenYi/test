...
class MODEL(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=21):
        ...
        ''' 简便的方法(利用def将模块包装起来)   
            但是会出现“AttributeError: 'MODEL' object has no attribute 'Conv_1x1_1'”的错误
            解决方法：要按照forward()中 层的书写顺序，在__init__()中写，不然会出现"object has no attribute"的错误！！！'''

        self.abw = nn.Sequential(
            ABW_BasicBlock(self.input_channels, self.input_channels, stride=1, padding=1, patch_size=patch_size),
        )
